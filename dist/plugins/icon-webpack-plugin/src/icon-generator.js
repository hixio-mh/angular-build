// Ref: Generate favicon - http://realfavicongenerator.net/api/non_interactive_api#.WHMaFxt96Ul
...
class IconGenerator {
    constructor() {
        ...
        this.safariPinnedTabDefault = {
            pictureAspect: 'blackAndWhite',
            threshold: 60
        };
        this.firefoxAppDefault = {
            pictureAspect: 'circle',
            keep_pictureInCircle: 'true',
            manifest: {}
        };
        this.yandexBrowserDefault = {
            manifest: {
                showTitle: true,
                version: '1.0'
            }
        ...
        this.openGraphDefault = {
            pictureAspect: 'backgroundAndMargin',
            margin: 0 // "12%"
        };
    }
    generateIcons(imageFileStream, iconsPath, online, preferOnline, options, cb) {
    ...
    // Offline
    //
    generateOffline(imageFileStream, iconsPath, options, cb) {
        const offlineOptions = this.prepareOfflineOptions(imageFileStream, options);
        favicons(imageFileStream, offlineOptions, (err, result) => {
            if (err) {
                cb(err);
                return;
            }
            const html = result.html.filter(entry => entry.indexOf('manifest') === -1)
    ...
    prepareOfflineOptions(imageFileStream, iconOptions) {
        const options = Object.assign({}, iconOptions);
    ...
    // Online
    //
    generateRfgOnline(imageFileStream, iconsPath, preferOnline, options, cb) {
        const rfgOptions = this.prepareRfgOnlineOptions(imageFileStream, options);
        ...
        client.post(RFG_API_URL, args, (data, response) => {
            ...
            const html = data.favicon_generation_result.favicon.html_code.split('\n');
    ...
    fetchRfgIconPack(url, cb) {
        const files = [];
        https.get(url, (res) => res.pipe(unzip.Parse())
            .on('entry', (entry) => {
    ...
    prepareRfgOnlineOptions(imageFileStream, iconOptions) {
        const options = Object.assign({}, iconOptions);
