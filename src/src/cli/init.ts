...
export function getInitCommandModule(): yargs.CommandModule {
    const initCommandModule: yargs.CommandModule = {
        ...
        builder: (yargv: yargs.Argv) => {
            ...
            const appConfigSchema: any = schema.definitions.AppConfigBase.properties;
...
export function init(cliOptions: CliOptions) : Promise<number> {
    ...
    return checkFileOrDirectoryExistsAsync(path.resolve(projectRoot, 'package.json'))
        ...
        .then(() => {
            if (cfg.useAngularCliConfigFile) {
                if (cfg.angularCliConfigFileExists && cfg.userAngularCliConfig) {
                    cfg.userAngularCliConfig.apps[0] = Object
                        .assign(cfg.userAngularCliConfig.apps[0], cfg.angularBuildConfig.apps[0]);
        ...
        .then(() => {
            if (cfg.webpackConfigFileExists && cfg.overrideWebpackConfigFile === false) {
                return Promise.resolve();
            } else {
                return new Promise((resolve, reject) => {
                            ...
                            if (exists) {
                                ...
                                return new Promise((resolve, reject) => {
                                    fs.copy(require.resolve('../../configs/environment.prod.ts'),
                                        path
                                        .resolve(projectRoot, appConfig.root, 'environments', 'environment.prod.ts'),
                                        err => {
                                            err ? reject(err) : resolve();
                                        });
                                });
...
function mergeConfigAsync(cfg: InitConfig) : Promise< void>{
    return mergeConfigWithPossibleAsync(cfg)
        ...
        .then(() => cfg.cliOptions.commandOptions &&
            (cfg.cliOptions.commandOptions.prompt && !cfg.cliOptions.commandOptions.force)
            ? mergeConfigWithPrompt(cfg)
            : Promise.resolve());
}
...
function mergeConfigWithAngularCliAsync(cfg: InitConfig): Promise<void> {
    const projectRoot = cfg.cliOptions.cwd;
        ...
        if (exists) {
            return readJsonAsync(cliPath).then((cliConfig: any) => {
                ...
                Object.keys(cliAppConfig)
                    .filter((key: string) => typeof cliAppConfig[key] !== 'undefined' && cliAppConfig[key] !== null)
                    .forEach((key: string) => {
...
function mergeWithCommandOptions(cfg: InitConfig) : Promise<void> {
...
function mergeConfigWithPrompt(cfg: InitConfig): Promise<void> {
    const projectRoot = cfg.cliOptions.cwd;
    const appConfig = cfg.angularBuildConfig.apps[0];
    ...
    return checkFileOrDirectoryExistsAsync(path.resolve(projectRoot, 'angular-build.json'))
        ...
        .then(() => {
            if (cfg.angularCliConfigFileExists && !cfg.useAngularCliConfigFile && !cfg.overrideAngularBuildConfigFile) {
                return askAsync(`Use 'angular-cli.json' as a build config yes/no (${
                        cfg.useAngularCliConfigFile ? 'yes' : 'no'})?: `)
                    .then((answer: string) => {
                        if (answer &&
                            answer.trim() &&
                            (answer.trim().toLowerCase() === 'yes' ||
                    ...
                    return askAsync(chalk.bgYellow('WARNING:') +
                        ...
                        .then((answer: string) => {
                            if (answer &&
                                ...
                                cfg.overrideWebpackConfigFile = false;
                            } else {
        ...
        .then(() => {
            if (cfg.cliOptions.commandOptions.root ||
                (!cfg.overrideAngularBuildConfigFile && cfg.angularBuildConfigFileExists) ||
                (cfg.angularCliConfigFileExists && cfg.useAngularCliConfigFile)) {
                return Promise.resolve(null);
            }
        ...
        .then(() => {
            if ((cfg.cliOptions.commandOptions.styles &&
                    cfg.cliOptions.commandOptions.styles.length &&
                    Array.isArray(cfg.cliOptions.commandOptions.styles)) ||
                (!cfg.overrideAngularBuildConfigFile && cfg.angularBuildConfigFileExists) ||
                (cfg.angularCliConfigFileExists && cfg.useAngularCliConfigFile)) {
                return Promise.resolve(null);
            }
            return askAsync(`Enter global css/scss/less/stylus style files ${
                    appConfig.styles
                    ? `(${Array.isArray(appConfig.styles) ? appConfig.styles.join(', ') : appConfig.styles})`
                    : ''}: `)
                .then((answer: string) => {
                    if (answer && answer.trim().length) {
                        ...
                        answerArray.forEach(style => {
                            ...
                            } else if (typeof (appConfig.styles as any) === 'string' &&
                                (appConfig.styles as any) !== style) {
...
function installToolings(cfg: InitConfig) : Promise<number>{
    const projectRoot = cfg.cliOptions.cwd;
