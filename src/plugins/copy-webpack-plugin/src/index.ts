    ...
    apply(compiler: webpack.Compiler): void {
        ...
        const emitFn = (compilation: any, cb: (err?: Error) => void) => {
            ...
            preProcessAssets(this._options.baseDir, this._options.assets, compilation.inputFileSystem)
                .then(async (preProcessedEntries) => processAssets(preProcessedEntries,
                    outputPath,
                    compilation.inputFileSystem))
                .then(async (processedAssets) => this.writeFile(processedAssets, compiler, compilation, outputPath || ''))
                .then(() => {
