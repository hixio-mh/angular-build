...
export async function processAssets(preProcessedEntries: PreProcessedAssetEntry[],
  outputPath?: string,
  inputFileSystem: any = require('fs')): Promise<ProcessedAssetsResult[]> {
  const results: ProcessedAssetsResult[] = [];
  await Promise.all(preProcessedEntries.map(async (assetEntry: PreProcessedAssetEntry) => {
    ...
    if (typeof assetEntry.from === 'string') {
      ...
      if (inputFileSystem.constructor.name === 'MemoryFileSystem') {
        ...
        const memFiles =
          getAllMemoryFiles(inputFileSystem, memItems, assetEntry.context, assetEntry.context);
        ...
        const foundPaths =
          await globPromise(fromGlobPattern, { cwd: assetEntry.context, nodir: true, dot: true });
    ...
    await Promise.all(relativeFromPaths.map(async (relativeFrom) => {
      ...
      const content = await new Promise<Buffer>((resolve, reject) => {
        inputFileSystem.readFile(absoluteFrom,
          (err: Error, data: Buffer) => err ? reject(err) : resolve(data));
      });
      ...
      if (assetEntry.to) {
        ...
        if (assetEntry.toIsTemplate && assetToEmit.relativeTo) {
          ...
          if (basename[0] === '.') {
            dotRemoved = true;
            assetToEmit.relativeFrom =
              path.join(path.dirname(assetToEmit.relativeFrom), basename.slice(1));
          }
...
export function getAllMemoryFiles(memoryFileSystem: any,
  items: string[],
  baseContext: string,
