const predefinedGlobalsMap: { [key: string]: string } = {
    tslib: 'tslib',
...
export const getRollupPredefinedGlobalsMap = (
    key: string
): { [key: string]: string } | null => {
    const fooundMap: { [key: string]: string } = {};
    let foundAny = false;
    const predefinedKeys = Object.keys(predefinedGlobalsMap);

    for (const predefinedKey of predefinedKeys) {
        if (key === predefinedKey || predefinedKey.startsWith(`${key}/`)) {
            fooundMap[predefinedKey] = predefinedGlobalsMap[predefinedKey];
            foundAny = true;
        }
    }

    return foundAny ? fooundMap : null;
};
